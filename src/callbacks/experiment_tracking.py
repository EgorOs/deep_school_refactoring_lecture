...
class LogConfusionMatrix(Callback):
    ...
    def on_test_epoch_end(self, trainer: Trainer, pl_module: LightningModule) -> None:
        targets = torch.cat(self.targets, dim=0).detach().cpu().numpy()  # noqa: WPS221
        predicts = torch.cat(self.predicts, dim=0).detach().cpu().numpy()  # noqa: WPS221
        cf_matrix = confusion_matrix(targets, predicts)
        cf_labels = [self.idx_to_class[idx] for idx, _ in enumerate(cf_matrix)]  # noqa: WPS221

        self.tracking_cb.task.logger.report_confusion_matrix(  # type: ignore
            'Confusion matrix',
            'ignored',
